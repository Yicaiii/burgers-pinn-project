...
def select_high_error_points(model, n_select, device):
    ...
    noise_x = 0.01 * np.random.randn(*x_new.shape)
    noise_t = 0.01 * np.random.randn(*t_new.shape)
...
def evaluate_error(model, device):
    x_grid, t_grid = build_eval_grid()
    u_pred = predict(model, x_grid, t_grid, device)
    u_ref = reference_solution(x_grid, t_grid, config.NU)
...
def main():
    ...
    print("Stage 1: training baseline active-learning v2 model...")
    loss_stage1 = train_model(
        model, x_f, t_f, x_b, t_b, u_b, x_i, t_i, u_i,
        epochs=epochs_stage1, lr=lr, device=device
    )

    mse_before, rel_l2_before = evaluate_error(model, device)
    print(f"Before enrichment - MSE: {mse_before:.6e}")
    print(f"Before enrichment - Relative L2 Error: {rel_l2_before:.6e}")

    x_add_error, t_add_error, _, _ = select_high_error_points(
        model, n_select=n_f_add_error, device=device
    )
    x_add_rand, t_add_rand = sample_collocation_uniform(n_f_add_random)

    x_f_enriched = np.vstack([x_f, x_add_error, x_add_rand])
    t_f_enriched = np.vstack([t_f, t_add_error, t_add_rand])

    print("Stage 2: retraining after mixed enrichment...")
    loss_stage2 = train_model(
        model, x_f_enriched, t_f_enriched, x_b, t_b, u_b, x_i, t_i, u_i,
        epochs=epochs_stage2, lr=lr, device=device
    )

    mse_after, rel_l2_after = evaluate_error(model, device)
    ...
    plot_losses(full_loss, "../outputs/figures/active_learning_v2_loss.png")

    torch.save(model.state_dict(), "../outputs/checkpoints/pinn_burgers_active_v2.pt")

    print("Saved loss curve to ../outputs/figures/active_learning_v2_loss.png")
    print("Saved model to ../outputs/checkpoints/pinn_burgers_active_v2.pt")
    print(f"Added points total: {n_f_add_total} = {n_f_add_error} error-guided + {n_f_add_random} random")
